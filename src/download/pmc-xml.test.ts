...
function createMockResponse(
  ...
  return {
    ...
    text:
      overrides.text ?? (() => Promise.resolve("<article><front></front><body></body></article>")),
    headers: {
...
describe("downloadPmcXml", () => {
  ...
  it("downloads XML from PMC E-utilities", async () => {
    const xmlContent =
      "<article><front><article-meta><title-group><article-title>Test</article-title></title-group></article-meta></front><body><p>Content</p></body></article>";
    ...
    expect(mockWriteFile).toHaveBeenCalledWith("/tmp/test/fulltext.xml", xmlContent, "utf-8");
  ...
  it("creates parent directory", async () => {
    ...
    expect(mockMkdir).toHaveBeenCalledWith("/tmp/test/nested", { recursive: true });
  });
